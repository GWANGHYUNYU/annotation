...
class FileFolderAnalysisDialog(QDialog, form_class):
    ...
    def showModal(self):
        ...
        for i in range(0, len(self.parent.species_list)):
            ...
            item_list = [class_str, species_str, species_cnt_str, leaf_cnt_str, flower_fruit_cnt_str, entire_cnt_str, multi_entire_cnt_str]
        ...
        for i in range(0, len(self.parent.class_list)):
            ...
            leaf_cnt_str = str(self.parent.class_leaf_cnt_list[i])
            flower_fruit_cnt_str = str(self.parent.class_flower_fruit_cnt_list[i])
            entire_cnt_str = str(self.parent.class_entire_cnt_list[i])
            multi_entire_cnt_str = str(self.parent.class_multi_entire_cnt_list[i])

            # 해당 어노테이션 정보에 대해서 부모 클래스의 어노테이션 디스플레이 테이블에 정보 추가
            item_list = [class_str, species_str, species_cnt_str, leaf_cnt_str, flower_fruit_cnt_str, entire_cnt_str,
        ...
        item_list = ['합계', '합계', str(self.parent.total_cnt), str(self.parent.total_leaf_cnt),
                     str(self.parent.total_flower_fruit_cnt),
                     str(self.parent.total_entire_cnt),
                     str(self.parent.total_multi_entire_cnt)]
